...
challenge_player.render_arena = function () {
    ...
    bifrost_match_users.forEach(function (user) {
        ...
        for (uid in challenge_player.match_data.gameDescription) {
            if (uid == user.uid) {
                username_font_attrs.fill = challenge_player.match_data.gameDescription[uid] == "0" ? "#cc0000" : "#0000cc";
            }
        }
...
challenge_player.render_tick = function (tick) {
    ...
    for (player in WM.players) {
        var player_color = player == 0 ? "#cc0000" : "#0000cc"

        for (body_idx in WM.players[player].body) {
